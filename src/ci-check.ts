import srcBoltDefault from "./tokens/bolt-default-list.json";
import boltDefaultList from "../lists/bolt-default-list.json";

const lists = [
  {
    name: "bolt-default-list",
    src: srcBoltDefault,
    actual: boltDefaultList,
  },
];
...
const compareLists = (listPair) => {
  ...
  if (src.length !== actual.tokens.length) {
    throw Error(
      `List ${name} seems to be not properly regenerated. Soure file has ${src.length} tokens but actual list has ${actual.tokens.length}. Did you forget to run yarn generate?`
    );
  }
  ...
  src.forEach((srcToken, index) => {
    if (JSON.stringify(srcToken) !== JSON.stringify(actual.tokens[index])) {
      throw Error(
        `List ${name} seems to be not properly regenerated. Tokens from src/tokens directory don't match up with the final list. Did you forget to run yarn generate?`
      );
    }
  });
};
